...
import { Printer, Download, Calendar, CheckCircle2, Loader2, School, GraduationCap, ArrowLeft } from 'lucide-react';
...
const ReportScreen: React.FC<ReportScreenProps> = ({ user, answers, onBack, autoDownload = false }) => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen font-sans print:bg-white">
      ...
      <div ref={reportRef} className="bg-gray-100 min-h-screen pt-20 pb-12"> 
        {/* HEADER */}
        <div className="bg-brand-navy text-white pt-8 pb-16 px-4 md:px-8 print:px-8 print:pt-4 print:pb-8">
        ...
        {/* MAIN CONTENT */}
        <div className="max-w-5xl mx-auto px-4 md:px-8 -mt-8 print:mt-0 print:px-0">
                    ...
                    <div className="flex flex-col md:flex-row gap-12 items-center">
                        ...
                        <div className="md:w-2/3">
                            ...
                            <p className="text-gray-600 leading-relaxed mb-4">
                                Your primary interest is <span className="font-bold text-brand-navy">{report.details[report.interestCode[0]].name}</span>. 
                ...
                <div className="p-8">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {report.interestCode.map((code, index) => {
                            ...
                            return (
                                <div key={code} className="bg-gray-50 rounded-xl p-6 border border-gray-100 relative overflow-hidden group hover:shadow-md transition-shadow">
                                    <div className="absolute top-0 left-0 w-full h-1" style={{ backgroundColor: color }}></div>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold shadow-sm" style={{ backgroundColor: color }}>
                                            {code}
                                        </div>
                                    ...
                                    <h4 className="text-lg font-bold text-gray-900 mb-3">{detail.name}</h4>
                                    ...
                                    <div className="space-y-3 pt-4 border-t border-gray-200/60">
                                        <div>
                                            ...
                                            <p className="text-xs font-medium text-gray-700">{detail.interests}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">Key Skills</p>
                                            <p className="text-xs font-medium text-gray-700">{detail.skills}</p>
                                        </div>
                                    </div>
                                </div>
                            );
            ...
            <div className="bg-brand-navy rounded-xl shadow-xl overflow-hidden text-white print-container print-break-inside print:bg-brand-navy print:text-white print:rounded-none">
                <div className="p-8 md:p-12">
                    ...
                    <div className="grid md:grid-cols-2 gap-12">
                        <div>
                            <h4 className="font-bold text-blue-200 uppercase text-xs tracking-widest mb-6">Recommended Streams</h4>
                            <div className="space-y-4">
                                {['Science (PCB) - Medical/Bio', 'Arts & Humanities', 'Commerce & Business'].map((stream, idx) => (
                                    <div key={idx} className="bg-white/5 rounded-lg p-4 border border-white/5 flex justify-between items-center">
                                        ...
                                        {idx === 0 && <span className="text-[10px] font-bold bg-brand-gold text-brand-navy px-2 py-0.5 rounded">HIGHLY RECOMMENDED</span>}
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div>
                            <h4 className="font-bold text-blue-200 uppercase text-xs tracking-widest mb-6">Best Fit Career Clusters</h4>
                            <div className="space-y-3">
                                {report.details[report.interestCode[0]].pathways.map((path, idx) => (
                                    <div key={idx} className="flex items-center gap-4">
                                        <div className="w-6 h-6 rounded-full bg-brand-gold/20 text-brand-gold flex items-center justify-center text-xs font-bold border border-brand-gold/30">
                                            {idx + 1}
                                        </div>
